...
class TestResearchOrchestrator:
    ...
    async def test_execute_batch(self, orchestrator: ResearchOrchestrator, mock_research_client: MockResearchClient):
        ...
        queries = [
            ResearchQuery(question=f"Question {i}", context="Context", priority=i)
            for i in range(5)
        ]
    ...
    async def test_concurrent_execution_limit(self, orchestrator: ResearchOrchestrator, mock_research_client: MockResearchClient):
        ...
        # Create many queries
        queries = [
            ResearchQuery(question=f"Query {i}", context="Context", priority=i)
            for i in range(10)
        ]
    ...
    async def test_race_condition_handling(self, orchestrator: ResearchOrchestrator, mock_research_client: MockResearchClient):
        """Test that race conditions in caching are handled correctly."""
        query = ResearchQuery(
        ...
        # Queue the same query multiple times concurrently
        await asyncio.gather(
            orchestrator.queue_research(query),
            orchestrator.queue_research(query),
            orchestrator.queue_research(query),
        )
